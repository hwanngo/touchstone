...
Full standard: **`standards/frameworks/react-native.md`** (layers on `standards/frameworks/react.md` +
`standards/languages/typescript.md`). The `standards/frameworks/react.md` state/component/forms rules
still hold; this covers what's native. Load-bearing rules:
...
- **Server state = TanStack Query** (persisted to MMKV, offline-tolerant, Zod-validated, timeout-bounded);
  client state = `useState`/Zustand — same split as `standards/frameworks/react.md`. Forms = React Hook Form + Zod.
