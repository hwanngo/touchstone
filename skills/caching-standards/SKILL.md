...
## Don't get burned
- **Stampede**: a hot key expiring melts the DB — TTL jitter + single-flight/coalescing, or early-recompute/stale-while-revalidate (standards/design/resilience.md §7).
...
- **Never cache PII/secrets/authz grants** in a shared cache — `private, no-store` for authenticated responses (standards/practices/data-privacy.md); re-check grants.
