...
*architecture* to [event-driven.md](../../standards/design/event-driven.md), pipeline instrumentation to
[observability.md](../../standards/platform/observability.md), and PII rules to
[data-privacy.md](../../standards/practices/data-privacy.md). Load-bearing rules:
...
- **Schema evolution is additive; breaking = a new version run in parallel** — dbt model contracts + producer data contracts (event-driven.md §8) gate downstream breakage.
- **Pipelines fail silently** — monitor the *data*: freshness, volume, distribution, schema. Green job + wrong data is the failure mode; alert on data, page on user impact (observability.md).
- **No raw PII in Gold/analytics** — minimize at ingest, mask in non-prod, and make GDPR erasure cascade into Bronze/Silver/Gold + backups (data-privacy.md §3).
