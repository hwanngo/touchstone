...
[architecture.md](../../standards/design/architecture.md) (sync vs async) and defers failure mechanics
to [resilience.md](../../standards/design/resilience.md). Load-bearing rules:
...
- **A poison message must never block the partition** — bounded backoff + jitter, then DLQ after N. Alert on DLQ depth > 0 and replay once fixed (resilience.md §7/§8).
- **Evolve schemas in a registry** (Avro/Protobuf) with backward+forward compat enforced in CI — additive-only, never renumber/retype; a breaking change is a new event version (api-design.md).
- **Propagate `traceparent` through the message** and alert on consumer *lag*, not just queue depth (devops.md).
